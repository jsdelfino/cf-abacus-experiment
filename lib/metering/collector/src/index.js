...
const wdebug = require('abacus-debug')('w-abacus-usage-collector');
...
// Return the watermark to periodically include in usage docs
let lastWatermark = 0;
const watermark = () => {
  const t = Date.now();
  if(lastWatermark === 0)
    lastWatermark = t;
  if(t < lastWatermark + 3000)
    return undefined;
  lastWatermark = t;
  wdebug('Watermark %d', t);
  return t;
};

...
const wdebug = require('abacus-debug')('w-abacus-usage-aggregator');
...
const aggregate = (a, u) => {
  ...
  const newa = extend(
    reviveOrg(JSON.parse(JSON.stringify(omit(a, 'watermark')))),
    {
...
const updateAggregatedUsage = function *(aid, udocs) {
  ...
  try {
    ...
    let newa = a ?
      omit(clone(a), 'watermark') : newOrg(udocs[0].u.organization_id);

...
const wdebug = require('abacus-debug')('w-abacus-usage-rate');
...
const rate = (r, u, pc) => {
  ...
  const newr = extend(clone(omit(r, 'watermark')), {
  ...
  if(newr.watermark)
    wdebug('Watermark %d, latency %d',
      newr.watermark, Date.now() - newr.watermark);
...
const updateRatedUsage = function *(rid, udocs) {
  ...
  const pc = yield pricingCountry(udocs[0].u.organization_id);
  ...
  try {
    ...
    let newr = r ? omit(clone(r), 'watermark') : {
      organization_id: udocs[0].u.organization_id,
      start: udocs[0].u.start,
      end: udocs[0].u.end
    };

...
const wdebug = require('abacus-debug')('w-abacus-usage-accumulator');
...
// Accumulate usage and return new accumulated usage
const accumulate = (a, u) => {
  // Merge new usage with accumulated usage
  const umerge = omit(u, 'id', 'metrics', 'metered_usage', 'measured_usage');
  const amerge = extend(a ? omit(clone(a), 'watermark') : {
    start: day(u.end),
    end: eod(u.end),
    accumulated_usage: []
  }, umerge);
  ...
  // Log watermark
  if(newa.watermark)
    wdebug('Watermark %d, latency %d',
      newa.watermark, Date.now() - newa.watermark);
...
const updateAccumulatedUsage = function *(aid, udocs) {
  ...
  try {
    ...
    let newa = omit(yield accumulatedUsage(aid), 'watermark');
    ...
    yield storeAccumulatedUsage(newa, aid, ludoc.alogid, ludoc.ulogid);
